...
public class TabBarWriter {
    ...
    static public void bootstrap(Writer writer, HttpServletRequest request, TabSet tabs, int level) throws IOException {
        ...
        for (int cc = 0; cc < tabs.getCount(); cc++) {
            ...
            boolean hasChild = tab.getCount() > 0;

            writer.write("<li");
            if (hasChild && level > 0) {
                writer.write(" class=\"dropdown-submenu\"");
            }
            writer.write(">");
            ...
            if (hasChild) {
                writer.write(" class=\"dropdown-toggle\" data-toggle=\"dropdown\"");
            }
            ...
            if (hasChild) {
                bootstrap(writer, request, tab, level + 1);
            }
    ...
    static public void nifty(Writer writer, HttpServletRequest request, TabSet tabs) throws IOException {
        ...
        for (int cc = 0; cc < tabs.getCount(); cc++) {
            TabSet tab = tabs.get(cc);
            boolean hasChild = tab.getCount() > 0;
            boolean hasAction = !DockYard.isWhiteSpace(tab.getAction());
            ...
            writer.write("<li");
            if (!hasAction) {
                writer.write(" class=\"list-header\"");
            }
            writer.write(">");

            if (hasAction) {
                writer.write("<a href=\"" + request.getContextPath() + tab.getAction() + "\">");
            }

            if (!DockYard.isWhiteSpace(tab.getToolTip())) {
                writer.write("<i class=\"fa fa-" + tab.getToolTip() + "\"></i>");
            }

            writer.write("<span class=\"menu-title\">");
            writer.write(tab.getText(request));
            writer.write("</span>");
    ...
    static public void nifty(Writer writer, HttpServletRequest request, TabSet tabs, int level) throws IOException {
        if (level > 1) {
            writer.write("<ul class=\"collapse\">");
        }
        ...
        for (int cc = 0; cc < tabs.getCount(); cc++) {
            ...
            if (DockYard.equalsIgnoreCase("divider", tab.getName())) {
                writer.write("<li class=\"list-divider\"></li>");
                continue;
            }

            writer.write("<li");
            if (tab.isSelected()) {
                writer.write(hasChild ? " class=\"active-sub active\"" : " class=\"active-link\"");
            }
            writer.write(">");
            ...
            if (hasChild) {
                nifty(writer, request, tab, level + 1);
            }
    ...
    static public void print(Writer writer, HttpServletRequest request, TabSet tabs, int level) throws IOException {
        ...
        writer.write(" class=\"level" + level + "\"");
        writer.write(">");
        ...
        for (int cc = 0; cc < tabs.getCount(); cc++) {
            ...
            writer.write("<li");
            if (hasChild) {
                writer.write(" class=\"child\"");
            }
            writer.write(">");

            if (DockYard.isWhiteSpace(tab.getAction())) {
                writer.write("<span class=\"separator\">");
            } else {
                writer.write("<a href=\"" + request.getContextPath() + tab.getAction() + "\">");
            }
            ...
            if (hasChild) {
                print(writer, request, tab, level + 1);
            }
