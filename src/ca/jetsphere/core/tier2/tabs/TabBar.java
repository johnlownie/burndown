...
public class TabBar extends ActionForm implements Serializable {
    ...
    public String getForward(TabSet tabs) {
        if (!DockYard.isWhiteSpace(tabs.getAction())) {
            return tabs.getAction();
        }
        ...
        while (it.hasNext()) {
            ...
            String action = getForward(child);
            if (!DockYard.isWhiteSpace(action)) {
                return action;
            }
        }
    ...
    public boolean setLevel0(TabSet tabs) {
        if (!DockYard.isWhiteSpace(tabs.getUuid()) && !tabs.isHidden()) {
            setLevel0(tabs.getUuid());
            return true;
        }
        ...
        while (it.hasNext()) {
            TabSet child = (TabSet) it.next();
            boolean hasAction = setLevel0(child);
            if (hasAction) {
                return true;
            }
        }
    ...
    public boolean setLevel1(TabSet tabs) {
        ...
        while (it.hasNext()) {
            TabSet child = (TabSet) it.next();
            if (!DockYard.isWhiteSpace(child.getAction())) {
                setLevel1(child.getUuid());
                return true;
            }
            if (setLevel1(child)) {
                return true;
