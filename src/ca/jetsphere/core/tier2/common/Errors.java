...
public class Errors extends Action {
     ...
     *
     */
    public Errors(ActionStore store) {
        this(store.getMapping(), store.getRequest());
    }
    ...
    public Errors add(String key, String value) {
        errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage(key, value));
        saveErrors();
        return this;
    }
    ...
    public Errors add(String key, String value, Object o) {
        errors.add(key, new ActionMessage(value, o));
        saveErrors();
        return this;
    }
    ...
    public Errors add(String key, String value, Object o, Object p) {
        errors.add(key, new ActionMessage(value, o, p));
        saveErrors();
        return this;
    }
    ...
    public void add(ActionErrors errors) {
        ...
        while (it.hasNext()) {
            this.errors.add(ActionErrors.GLOBAL_MESSAGE, (ActionMessage) it.next());
        }
    }
    ...
    public void clearErrors() {
        if (request != null) {
            saveMessages(request, errors = new ActionErrors());
        }
    }
    ...
    public ActionForward forward() {
        saveErrors();
        return mapping != null ? mapping.findForward("failure") : null;
    }

    /**
     *
     */
    public ActionForward forward(ActionMessage error) {
        add(error);
        return forward();
    }
     ...
     *
     */
    public ActionForward forward(ActionMapping mapping) {
        return mapping.findForward(isEmpty() ? "success" : "failure");
    }

    /**
     *
     */
    public ActionForward forward(String error) {
        add(error);
        return forward();
    }
     ...
     *
     */
    public ActionForward forward(String error, Object o) {
        add(error, o);
        return forward();
    }
     ...
     *
     */
    public ActionForward forward(ActionStore store) {
        return forward(store.getMapping());
    }
    ...
    public JSONObject getFields(HttpServletRequest request) {
        ...
        while (it.hasNext()) {
            ActionMessage message = (ActionMessage) it.next();
            fields.put(message.getKey(), Caption.get(request, (String) message.getValues()[0]));
        }
    ...
    public void saveErrors() {
        if (request != null && !errors.isEmpty()) try {
