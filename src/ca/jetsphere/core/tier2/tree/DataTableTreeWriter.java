...
public class DataTableTreeWriter {
    ...
    public DataTableTreeWriter(TreeSession trees, String[] headers, String[] fields) {
        ...
        this.datatable_json_object = new JSONObject();
    }
    ...
    protected void branch(JSONArray array, OpenSet openSet, Tree tree) {
        ...
        if (!openSet.contains(tree.getId())) {
            return;
        }
        ...
        for (int i = 0; i < tree.getChildCount(); i++) {
            ...
            if (child.isLeaf()) {
                leaf(array, openSet, child);
            } else {
                branch(array, openSet, child);
            }
        }
    ...
    protected JSONArray getData(OpenSet openSet, Tree tree) {
        ...
        if (tree.isLeaf()) {
            leaf(jsonArray, openSet, tree);
        } else {
            branch(jsonArray, openSet, tree);
        }
    ...
    public JSONObject getJsonObject() {
        return this.datatable_json_object;
    }

    public TableModel getTableModel() {
        return this.datatable_table_model;
    }
    ...
    public int getStart() {
        return this.datatable_start;
    }

    public int getLength() {
        return this.datatable_length;
    }
    ...
    public void print(Writer writer, HttpServletRequest request, OpenSet openSet) {
        try {
            ...
            this.datatable_length = getTableModel().getRows();
    ...
    public void print(Writer writer, HttpServletRequest request, int dt_draw, int dt_count, int dt_filtered, int dt_start, int dt_length) {
        try {
            ...
            this.writer = writer;
            this.request = request;
            this.datatable_draw = dt_draw;
            this.datatable_start = dt_start;
            this.datatable_length = dt_length;
