...
public class TableWriter {
     ...
     *
     */
    public TableWriter(TableModel table_model_model) {
        setModel(table_model_model);
        setCaption("");
    }
    ...
    protected void captions() {
        if (DockYard.isWhiteSpace(getCaption())) {
            return;
        }
    ...
    protected void header(int column) {
        write("<th>");
        write(getCaption(request, column));
        write("</th>");
    }
    ...
    protected void rows() {
        write("<tbody>");
        if ((getModel().getHeaders() == null || getModel().getHeaders().length == 0) && getModel().getRows() == 0) {
            emptySet();
        }
    ...
    public void table(String table_id) {
        setCaption();
        write("<div class=\"table-responsive\"><table id=\"" + table_id + "\" class=\"table table-striped table-hover nowrap\" cellspacing=\"0\">");
        captions();
        headers();
        if (!headers_only) {
            rows();
    ...
    public TableModel getModel() {
        return table_model_model;
    }

    public String getCaption() {
        return table_model_caption;
    }

    /**
     *
     */
    public void setModel(TableModel table_model_model) {
        this.table_model_model = table_model_model;
    }

    public void setCaption(String table_model_caption) {
        this.table_model_caption = table_model_caption;
    }
