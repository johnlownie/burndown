...
public class UserEditAction extends AbstractEditAction {
    ...
    final protected void validate(JDBC jdbc, HttpServletRequest request, User user, Errors errors) throws Exception {
        ...
        if (!user.isValid() && existing.isValid()) {
            errors.add("error.username.invalid");
            return;
        }

        if (user.isValid() && existing.isValid() && user.getId() != existing.getId()) {
            errors.add("error.username.invalid");
            return;
        }

        if (DockYard.isWhiteSpace(user.getNewPassword()) && !user.isValid()) {
            errors.add("error.password.required");
            return;
        }

        if (DockYard.compareTo(user.getNewPassword(), user.getConfirmPassword()) != 0) {
            errors.add("error.password.change.nomatch");
            return;
        }

        if (!user.isValid() || !DockYard.isWhiteSpace(user.getNewPassword())) {
