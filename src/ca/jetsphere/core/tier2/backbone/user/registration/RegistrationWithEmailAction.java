...
public class RegistrationWithEmailAction extends UserEditAction {
    ...
    public ActionForward fetch(JDBC jdbc, ActionStore store, Errors errors) throws Exception {
        try {
            ...
            if (errors.isEmpty()) {
                RegistrationYard.send(jdbc, store.getRequest(), applicant, errors);
            }
    ...
    protected void setCompanyMember(JDBC jdbc, HttpServletRequest request, User user) throws Exception {
        ...
        companyMember.setFirstname(DockYard.getToken(user.getNotes(), 1, " "));
        companyMember.setLastname(DockYard.getRemaining(user.getNotes(), " "));
    ...
    public Errors update(JDBC jdbc, HttpServletRequest request, Bolt bolt, Errors errors) throws Exception {
        ...
        try {
            ...
            applicant.setNotes(applicant.getFullname());
    ...
    public ActionForward verify(JDBC jdbc, ActionStore store, Errors errors) throws Exception {
        ...
        if (DockYard.isWhiteSpace(uuid)) {
            errors.add("error.registration.invalid.key");
            return store.getForward("invalid", "", false);
        }

        Token token = TokenYard.getByUuid(jdbc, uuid);

        if (token == null) {
            errors.add("error.registration.expired.key");
            return store.getForward("invalid", "", false);
        }

        User user = new User(jdbc, token.getIdentifier());

        if (user == null || !user.isValid() || user.getStatus() != Status.PENDING) {
            return store.getForward("invalid", "", false);
        }

        if (isResend) {
            RegistrationYard.resend(jdbc, store.getRequest(), user, errors);
            errors.add("register.email.resent");
            return store.getForward("invalid", "", false);
        }

        setCompanyMember(jdbc, store.getRequest(), user);
        setDefaultRole(jdbc, store.getRequest(), user);
        user.setNotes("");
