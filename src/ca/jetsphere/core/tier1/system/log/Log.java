...
public class Log extends Bolt {
    ...
    public Log(int id, File file) throws Exception {
        ...
        setSize(file.length());
        setUrl(file.getCanonicalPath());
        setLastUpdate(new Timestamp(file.lastModified()));
    }
    ...
    static public String[] captions() {
        return new String[]{"name", "size", "last.update", "actions"};
    }
    ...
    static public String[] fields() {
        return new String[]{"log_name", "log_size", "log_last_update", "log_uuid"};
    }
    ...
    public Object get(String s) {
        if (DockYard.compareTo("log_uuid", s) == 0) {
            return getEditDeleteBox("" + getId());
        }

        if (DockYard.compareTo("log_size", s) == 0) {
            return DockYard.getDecimalFormat(getSize(), "###,###,##0");
        }
    ...
    static public String getLogName() throws Exception {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                int cc = line.indexOf("/logs/");
                if (cc == -1) {
                    continue;
                }
                line = line.substring(cc + 6);
                cc = line.indexOf(".log");
                if (cc != -1) {
    ...
    public void setText(String log_text) {
        this.log_text = log_text;
    }

    public void setSize(long log_size) {
        this.log_size = log_size;
    }

    public void setUrl(String log_url) {
        this.log_url = log_url;
    }

    public void setLastUpdate(Timestamp log_last_update) {
        this.log_last_update = log_last_update;
    }
