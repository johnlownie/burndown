...
public class CalendarEventYard {
    ...
    static public boolean contains(List dates, String date) {
        try {
            ...
            while (it.hasNext()) {
                ...
                Date start = CalendarYard.getDate(event.getStart(), "yyyy-MM-dd");
                Date end = CalendarYard.getDate(event.getEnd(), "yyyy-MM-dd");
    ...
    static public List filterDates(List set, List subSet) {
        ...
        while (it.hasNext()) {
            ...
            if (contains(set, calendarEvent.getStart())) {
                it.remove();
            }
        }
    ...
    static public CalendarEvent get(String start_date, String end_date) {
        ...
        event.setBackgroundColor("#ddd");
        event.setBorderColor("#ddd");
    ...
    static public List getBackgroundEvents(List<String> days, String color, boolean ignore_weekends) {
        ...
        for (String day : days) {
            if (DockYard.isWhiteSpace(day) || (ignore_weekends && !CalendarYard.isWeekday(day))) {
                continue;
            }
    ...
    static public List getSCUnavailableDates(int cutoff) {
        List list = new ArrayList();
        String start = "2017-09-04";
        String next_monday = nextMonday("yyyy-MM-dd", cutoff > Calendar.SUNDAY && cutoff < Calendar.SATURDAY ? cutoff + 1 : Calendar.THURSDAY);
    ...
    static public List noCommon(List<String> available_days, List<CalendarEvent> unavailable_days) {
        ...
        for (String date : available_days) {
            if (!contains(unavailable_days, date)) {
                list.add(date);
            }
        }
