...
public class RoleSession extends RoleMap implements ISessionObject {
    ...
    public void clearSelected(HttpServletRequest request) {
        setNonQualifiedSelected(request, new Role());
    }
     ...
     *
     */
    static public Role getSelected(HttpServletRequest request) {
        return (Role) getInstance(request).getSelected();
    }
    ...
    static public RoleSession query(JDBC jdbc, HttpServletRequest request, boolean blank) {
        RoleSession session = getInstance(request);
        session.find(jdbc, CompanySession.getSelected(request));
...
//    { RoleSession session = RoleSession.query ( jdbc, request, blank ); RoleYard.inclusiveFilter ( session, type ); session.options ( request, blank ); return session; }
    ...
    static public RoleSession query(JDBC jdbc, HttpServletRequest request, int type, String alias, boolean blank) {
        RoleSession session = RoleSession.query(jdbc, request, blank);
        RoleYard.inclusiveFilter(session, type);
        RoleYard.inclusiveFilter(session, alias);
        session.options(request, blank);
        return session;
    }
     ...
     *
     */
    static public void setSelected(HttpServletRequest request, Role selected) {
