...
public class Role extends ca.jetsphere.core.tier0.backbone.role.Role {
     ...
     *
     */
    public Role(JDBC jdbc, int role_id) {
        super(jdbc, role_id);
    }
    ...
    public Role(JDBC jdbc, int application_id, String alias) throws Exception {
        this();
        query(jdbc, "select * from jet_base_role where role_application_id = " + application_id + " and role_alias = " + DockYard.quote(alias));
    }
    ...
    static public String[] captions() {
        return new String[]{"name", "type", "read.only", "last.update", "actions"};
    }
    ...
    static public String[] fields() {
        return new String[]{"role_name", "role_type", "role_read_only", "role_last_update", "role_uuid"};
    }
    ...
    public Object get(String s) {
        if ("role_name".equals(s)) {
            return Caption.get(getLocale(), getName());
        }
        if ("role_type".equals(s)) {
            return Caption.get(getLocale(), Type.getType(getType()));
        }
    ...
    static public int getId(JDBC jdbc, HttpServletRequest request, String role_alias) throws Exception {
        ...
        if (!role.isValid()) {
            role = new Role(jdbc, -1, role_alias);
        }
    ...
    static public boolean isType(JDBC jdbc, int role_type, int role_id) throws Exception {
        ...
        return (role.getType() & role_type) != 0;
    }
