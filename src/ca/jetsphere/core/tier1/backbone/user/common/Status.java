...
public class Status {
     ...
     *
     */
    static public String get(HttpServletRequest request, int status) {
        return Caption.get(request, get(status));
    }

    /**
     *
     */
    static public int get(HttpServletRequest request) {
        return DockYard.toInteger(request, key());
    }
    ...
    static public int inverse(HttpServletRequest request, String status) {
        if (DockYard.compareTo(get(request, ACTIVE), status) == 0) {
            return ACTIVE;
        }

        if (DockYard.compareTo(get(request, FROZEN), status) == 0) {
            return FROZEN;
        }

        if (DockYard.compareTo(get(request, CLOSED), status) == 0) {
            return CLOSED;
        }

        if (DockYard.compareTo(get(request, PENDING), status) == 0) {
            return PENDING;
        }
