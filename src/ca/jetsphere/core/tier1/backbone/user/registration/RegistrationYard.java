...
public class RegistrationYard {
    ...
    static public void resend(JDBC jdbc, HttpServletRequest request, User user, Errors errors) {
        ...
        if (!errors.isEmpty()) {
            errors.add("error.registration.resend.email");
            return;
        }
    }
    ...
    static public void send(JDBC jdbc, HttpServletRequest request, User user, Errors errors) {
        ...
        if (!errors.isEmpty()) {
            errors.add("username", "error.registration.resend.email");
            return;
        }
    }
    ...
    static public void sendVerificationMessage(JDBC jdbc, HttpServletRequest request, User user, String token_uuid, Errors errors) {
        VerificationMessage verificationMessage = new VerificationMessage(jdbc, request, errors);
        verificationMessage.set(user, token_uuid);

        if (errors.isEmpty()) {
            SendMailBrokerYard.send(request.getServletContext(), verificationMessage);
        }
    }
