...
public class RegisterValidate {
    ...
    protected void collision(JDBC jdbc, User applicant, Errors errors) {
        ...
        if (x.isValid()) {
            errors.add("username", "error.registration.username.invalid");
        }
    }
    ...
    protected void mandatory(User applicant, Errors errors) {
        if (DockYard.isWhiteSpace(applicant.getFullname())) {
            errors.add("fullname", "error.registration.fullname.required");
        }

        if (DockYard.isWhiteSpace(applicant.getUsername())) {
            errors.add("username", "error.registration.username.required");
        }

        if (DockYard.isWhiteSpace(applicant.getNewPassword())) {
            errors.add("password", "error.registration.password.required");
        }

        if (DockYard.isWhiteSpace(applicant.getPhone())) {
            errors.add("phone", "error.registration.phone.required");
        }
    }
    ...
    protected void recaptcha(String response, Errors errors) {
        ...
        if (!valid) {
            Common.trace("Invalid Captcha");
            errors.add("captchaContainer", "error.registration.recaptcha.invalid");
        }
    }
    ...
    public Errors validate(JDBC jdbc, User applicant, String response, Errors errors) {
        mandatory(applicant, errors);
        if (!errors.isEmpty()) {
            return errors;
        }

        if (!DockYard.toBoolean(Knock.get("DEVELOPER"))) {
            recaptcha(response, errors);
            if (!errors.isEmpty()) {
                return errors;
            }
        }

        collision(jdbc, applicant, errors);
        if (!errors.isEmpty()) {
            return errors;
        }

        confirm(applicant, errors);
        if (!errors.isEmpty()) {
            return errors;
        }
