...
public class User extends ca.jetsphere.core.tier0.backbone.user.User {
     ...
     *
     */
    public User(JDBC jdbc, int user_id) {
        super(jdbc, user_id);
    }
     ...
     *
     */
    public User(JDBC jdbc, ResultSet rs) throws Exception {
        this();
        get(jdbc, rs);
    }
    ...
    static public String[] captions() {
        return new String[]{"username", "status", "role", "application", "last.login", "actions"};
    }
    ...
    public void foreign(JDBC jdbc) throws Exception {
        setCompanyMember(CompanyMemberYard.getByUserId(jdbc, getId()));
    ...
    public Object get(String s) {
        if ("user_status".equals(s)) {
            return Caption.get(getLocale(), Status.get(getStatus()));
        }

        if ("user_roles".equals(s)) {
            return RoleYard.getRoleNames(getRoleIds());
        }

        if ("user_application_id".equals(s)) {
            return ApplicationYard.getName(getApplicationId());
        }
    ...
    public int getCompanyId() {
        return companyMember == null || !companyMember.isValid() ? -1 : companyMember.getCompanyId();
    }
    ...
    public int getCompanyMemberId() {
        return companyMember == null || !companyMember.isValid() ? -1 : companyMember.getId();
    }

    public String getFirstname() {
        return companyMember == null || !companyMember.isValid() ? "???" : companyMember.getFirstname();
    }

    public String getLastname() {
        return companyMember == null || !companyMember.isValid() ? "???" : companyMember.getLastname();
    }
    ...
    public String getName() {
        return getCompanyMember().getName() + " (" + getUsername() + ")";
    }
    ...
    public void save(JDBC jdbc) throws Exception {
        if (!DockYard.isWhiteSpace(getRoles())) {
            setRoleIds(String.join(", ", getRoles()));
        }
        super.save(jdbc);
    }
     ...
     *
     */
    public void setConfirmPassword(String user_confirm_password) {
        this.user_confirm_password = user_confirm_password;
    }

    public void setConfirmUsername(String user_confirm_username) {
        this.user_confirm_username = user_confirm_username;
    }
    ...
    public void setNewPassword(String user_new_password) {
    ...
    public void setRoles(String[] user_roles) {
        this.user_roles = user_roles;
    }

    public void setIsAdministrator(boolean user_is_administrator) {
