...
public class Audit extends ca.jetsphere.core.tier0.backbone.audit.Audit {
    ...
    static public String[] captions() {
        return new String[]{"name", "audit.ip", "audit.session.id", "audit.path", "audit.query.string", "audit.method", "audit.errors", "last.update"};
    }
    ...
    public void foreign(JDBC jdbc) {
        if (getMemberId() > 0) {
            setUsername(new User(jdbc, getMemberId()).getUsername());
        }
    }
    ...
    public Object get(String s) {
        if ("audit_member_id".equals(s)) {
            return getMemberId() > 0 ? getUsername() : "";
        }
    ...
    public void stop() {
        setStop(now());
        setElapsed(stopWatch.getElapsed());
    }
     ...
     *
     */
    public void setUsername(String audit_username) {
    ...
    static public boolean shouldAudit(HttpServletRequest request) {
        if (DockYard.compareTo(request.getServletPath(), "/ajax_lockout.do") == 0) {
            return false;
        }
