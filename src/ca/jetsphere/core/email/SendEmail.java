...
public class SendEmail {
    ...
    List cc;
    List bcc;
    String from, signature, subject;

    /**
     *
     */
    private SendEmail() {
        setConnection(new SmtpConnection());
        setTo(new ArrayList());
        setCc(new ArrayList());
     ...
     *
     */
    public SendEmail(String from, String to, String subject) {
        this(from, from, to, subject);
    }

    /**
     *
     */
    public SendEmail(String from, List to, String subject) {
        this(from, from, to, subject);
    }
    ...
    public void addRecipients(Message message) throws Exception {
        Iterator it = getTo().iterator();
        while (it.hasNext()) {
            addTo(message, (String) it.next());
        }

        it = getCc().iterator();
        while (it.hasNext()) {
            addCc(message, (String) it.next());
        }
    }
    ...
    public String getAddresses(Message message) throws Exception {
        ...
        for (int cc = 0; cc < address.length; cc++) {
            sb.append(address[cc].toString()).append(" ");
        }
    ...
    public MimeMessage getMimeMessage() {
        return new MimeMessage(connection.getSession());
    }
    ...
    public MimeBodyPart getTextBodyPart(String text, boolean html) throws Exception {
        ...
        if (html) {
            mps.setContent(text, "text/html; charset=utf-8");
        } else {
            mps.setText(text, "UTF-8");
        }
    ...
    public void send(ca.jetsphere.core.email.Message message) throws Exception {
        if (DockYard.isWhiteSpace(getFrom())) {
     ...
     *
     */
    public void send(String text, FileAttachmentList list) throws Exception {
        send(text, list, false);
    }
