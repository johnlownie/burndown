...
public class SendMailBroker implements Runnable {
     ...
     */
    public void trace() {
        Common.debug("[ SEND MAIL BROKER ] ( QUEUE SIZE ) " + queue.size());
    }
    ...
    public void whirl() throws Exception {
        ...
        try {
            ...
            if (fal == null || fal.isEmpty()) {
                sendEmail.send(x.getMessage(), true);
            } else {
                sendEmail.send(x.getMessage(), fal, true);
            }
