...
public class MaintenanceModeFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException, ServletException {
        ...
        User whoAmI = UserYard.whoAmI(httpRequest);

        if (request.getParameter("maintenance-mode") != null) {
            setMaintenance(request, response, filterChain);
        ...
        if (mode == MODE_NO_MORE_LOGINS && "/login.do".equals(path)) {
            request.getRequestDispatcher("/maintenance.do").forward(request, response);
            return;
        }

        if (mode == MODE_NO_WIZARDS && DockYard.contains(path, "wizard")) {
            request.getRequestDispatcher("/nowizard.do").forward(request, response);
            return;
        }

        if (mode == MODE_SUSPENSION) {
            request.getRequestDispatcher("/maintenance.do").forward(request, response);
            return;
        }
    ...
    public void setMaintenance(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException, ServletException {
        ...
        message = request.getParameter("maintenance-message");
    }
