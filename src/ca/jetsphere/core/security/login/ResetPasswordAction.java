...
//    sb.append ( "<p>" + Caption.get ( request, "password.reset.sent.email.3", new String[] { link } ) + "</p>" );
    ...
    public ActionForward fetch(JDBC jdbc, ActionStore store, Errors errors) throws Exception {
        try {
            ...
            if (fields.isEmpty()) {
                User x = UserYard.getByEmail(jdbc, user.getUsername());
                String token = TokenYard.get(jdbc, x.getId(), 30);
                ...
                ResetPasswordMessage message = new ResetPasswordMessage(jdbc, store.getRequest(), errors);
                message.set(user, token);

                if (errors.isEmpty()) {
                    SendMailBrokerYard.send(store.getRequest().getServletContext(), message);
                }
            }
    ...
    public void setup(JDBC jdbc, HttpServletRequest request, Bolt bolt, Errors errors) throws Exception {
        ...
        if (token == null) {
            errors.add("error.password.reset.expired.key");
            return;
        }
        ...
        form.copy(user);
        form.setPassword(PasswordHash.createHash(form.getNewPassword()));
    }
    ...
    protected JSONObject validate(JDBC jdbc, HttpServletRequest request, User user) {
        ...
        if (!EmailYard.isValid(address)) {
            jsonObject.put("username", Caption.get(request, "error.password.reset.invalid.email"));
            return jsonObject;
        }
    ...
    public ActionForward verify(JDBC jdbc, ActionStore store, Errors errors) throws Exception {
        ...
        if (DockYard.isWhiteSpace(uuid)) {
            errors.add("error.password.reset.invalid.key");
            return store.getForward("invalid", "", true);
        }
        ...
        if (user == null || !user.isValid()) {
            return store.getForward("invalid");
        }

        if (isResend) {
            return store.getForward("success", "?token=" + uuid);
        }
