...
public class LockOut {
     ...
     */
    public long elapsed() {
        return isWait() ? System.currentTimeMillis() - getWatch() : 0;
    }
    ...
    public String getAlert(HttpServletRequest request) {
        if (isWait() && isWarn()) {
            return Caption.get(request, "server.will.shut.down.in", getTime(shutdown()));
        }
        if (isLock()) {
            return Caption.get(request, "server.is.stopped", getTime(stopped()));
        }
        return "";
    }
    ...
    public int getWarningMinutes() {
        int minutes = DockYard.toInteger(Knock.get("LOCKOUT-WARNING-MINUTES"));
        if (minutes < 0) {
            minutes = 15;
        }
        return minutes;
    ...
    public boolean isWarn() {
        return getWarningMinutes() * 60 * 1000 >= shutdown();
    }
    ...
    public long shutdown() {
        return isWait() ? getMillis() - elapsed() : 0;
    }
     ...
     */
    public long stopped() {
        return isLock() ? System.currentTimeMillis() - getWatch() : 0;
    }
    ...
    private void setWatch(long watch) {
        this.watch = watch;
    }
