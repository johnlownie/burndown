...
public class Clock extends ActionForm {
     ...
     *
     */
    public Clock(String key, String format, boolean now) {
        this(key);
        if (now) {
            set(now());
        }
     ...
     *
     */
    public Clock(String key, HttpServletRequest request) {
        this(key);
        options(request);
    }
     ...
     *
     */
    public Clock(String key, Timestamp ts) {
        this(key);
        set(ts);
    }
     ...
     *
     */
    public Clock(String key, Timestamp ts, String format) {
        this(key);
        set(ts);
        setFormat(format);
    }
    ...
    public void options(HttpServletRequest request) {
        int hh = DockYard.toInteger(request, getKey() + ":hours");
        if (hh < 0) {
            hh = 0;
        }
        setHours(hh);

        int mm = DockYard.toInteger(request, getKey() + ":minutes");
        if (mm < 0) {
            mm = 0;
        }
        setMinutes(mm);

        int ss = DockYard.toInteger(request, getKey() + ":seconds");
        if (ss < 0) {
            ss = 0;
        }
        setSeconds(ss);
    ...
    public void print(Writer writer, HttpServletRequest request, boolean enabled, boolean submit) throws Exception {
        if (format.indexOf("hh") > -1) {
            hh(writer, enabled, submit);
        }

        if (format.indexOf("mm") > -1) {
            mm(writer, enabled, submit);
        }

        if (format.indexOf("ss") > -1) {
            ss(writer, enabled, submit);
        }
    }
