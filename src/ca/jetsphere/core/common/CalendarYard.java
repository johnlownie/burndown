...
public class CalendarYard {
    ...
    static public String getClosestDay(String[] dates, String date) {
        if (DockYard.isWhiteSpace(date) || dates == null || dates.length == 0) {
            return null;
        }
    ...
    static public List getDaysOfWeek(String date) {
        ...
        LocalDate endDate = localDate.with(DayOfWeek.FRIDAY).plusDays(1);

        while (startDate.isBefore(endDate)) {
            String d = startDate.format(formatter);
            if (DockYard.compareTo(date, d) != 0) {
    ...
    static public String formatDate(String in_date, String in_format, String out_format) {
        try {
            ...
            if (DockYard.isWhiteSpace(in_date)) {
                return "";
            }
    ...
    static public Date getDate(String s, String format) {
        try {
            ...
            Common.trace("Error - unable to parse " + s + " (" + format + ")");
            return null;
        }
    ...
    static public int getDateAsInt(String s, String format) {
        try {
            ...
            String fDate = new SimpleDateFormat("MMdd").format(date);
     ...
     *
     */
    static public String getDateFormatDateTimeMedium(Locale locale, Date date) {
        return getDateFormatDateTime(locale, date, DateFormat.MEDIUM);
    }
    ...
    static public String getDateTimeFormat(Date date, String s) {
        ...
        return DockYard.isWhiteSpace(s) ? getDateTimeFormat(date) : getDateTimeFormat(getSimpleDateFormat(s), date);
    }
    ...
    static public List getDayList(HttpServletRequest request, String blank) {
        ...
        if (!DockYard.isWhiteSpace(blank)) {
            list.add(new LabelValueBean(blank, "-1"));
        }
    ...
    static public List getDaysAround(String date) {
        ...
        LocalDate startDate = initial.with(firstDayOfMonth());
        LocalDate endDate = initial.with(lastDayOfMonth());

        long days = Period.between(startDate, endDate).getDays();

        for (long cc = 0; cc < days; cc++) {
            LocalDate d = startDate.plusDays(cc);
            dates.add(d);
        }
    ...
    static public String getFirstDayOfMonth(String date) {
        return getFirstDayOfMonth(date, "yyyy-MM-dd");
    }
    ...
    static public String getFridayOnlyDelivery() {
        ...
        switch (now.getDayOfWeek()) {
            case TUESDAY:
                return now.getHour() < 23 ? nextFriday.format(formatter) : followingFriday.format(formatter);
            case WEDNESDAY:
            case THURSDAY:
                return followingFriday.format(formatter);
            default:
    ...
    static public List getMondays(HttpServletRequest request) {
        ...
        while (thisMonday.isBefore(endDate)) {
            list.add(new LabelValueBean(Caption.get(request, "menu.availability.week.of", new String[]{thisMonday.format(formatter)}), thisMonday.toString()));
     ...
     *
     */
    static public String getMonthAsString(String month) {
        switch (month) {
            case "JAN":
                return "01";
            case "FEB":
    ...
    static public List<String> getMonthsBetween(String start_date, String end_date, String in_format, String out_format) {
        ...
        DateFormat dateFormat = getSimpleDateFormat("MMM");

        Date startDate = getDate(start_date, "yyyy-MM-dd");
        Date endDate = getDate(end_date, "yyyy-MM-dd");
    ...
    static public Pair[] getMonthDates(String start_date, int num_of_months) {
        ...
        for (int i = 0; i < num_of_months; i++) {
            String month_start = getFirstDayOfMonth(start_date);
            String month_end = getLastDayOfMonth(start_date);
    ...
    static public String[] getNextWeek(String date) {
        ...
        localDate = localDate.plusWeeks(1).with(DayOfWeek.MONDAY);
        days[0] = localDate.format(formatter);

        localDate = localDate.with(DayOfWeek.TUESDAY);
        days[1] = localDate.format(formatter);

        localDate = localDate.with(DayOfWeek.WEDNESDAY);
        days[2] = localDate.format(formatter);

        localDate = localDate.with(DayOfWeek.THURSDAY);
        days[3] = localDate.format(formatter);

        localDate = localDate.with(DayOfWeek.FRIDAY);
        days[4] = localDate.format(formatter);
     ...
     *
     */
    static public java.util.TimeZone getTimeZone() {
        return TimeZone.getTimeZone(Knock.get("TIMEZONE"));
    }
    ...
    static public String getTwoDayDelivery() {
        ...
        switch (now.getDayOfWeek()) {
            ...
            case SATURDAY:
            case SUNDAY:
                return nextThursday.format(formatter);
            case MONDAY:
                return followingTuesday.format(formatter);
            case TUESDAY:
            case WEDNESDAY:
                return nextTuesday.format(formatter);
            default:
                return nextThursday.format(formatter);
        }
    ...
    static public String getUniqueDaysOfWeek(String[] dates) {
        ...
        for (String date : dates) {
            LocalDate localDate = LocalDate.parse(date, formatter);
            set.add(localDate.getDayOfWeek().toString().substring(0, 3));
        }
    ...
    static public List<LabelValueBean> getWeeksOfDates(HttpServletRequest request, List<Date> dates) {
        ...
        Collections.sort(dates, BY_WEEK);
        int current = -1;
        ...
        for (Date date : dates) {
            ...
            if (current != getWeekOfYear(date)) {
                weeks.add(new LabelValueBean(Caption.get("week.of", new String[]{getMondayOfDate(date, "MMM dd")}), "" + week_of_year));
    ...
    static public boolean isAfter(String date, String format, int days) {
        ...
        LocalDate now = LocalDate.now();
        LocalDate then = now.plusDays(days);
     ...
     *
     */
    static public boolean isMonday() {
