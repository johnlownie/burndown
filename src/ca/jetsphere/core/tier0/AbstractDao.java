...
abstract public class AbstractDao {
    ...
    protected boolean delete(JDBC jdbc, Bolt bolt, String query) throws Exception {
        ...
        try {
            ...
            Common.debug("[ AbstractDao:delete ] ( >> ) " + bolt.getKey());
            Common.debug(" [ Query ] " + query);
            statement = jdbc.createStatement();
            int rowCount = statement.executeUpdate(query);
            Common.debug("[ AbstractDao:delete ] ( << ) " + rowCount);
            return rowCount > 0;
    ...
    public void insert$(JDBC jdbc, Bolt bolt, PreparedStatement ps) throws Exception {
        ...
        try {
            ...
            if (rs.next()) {
                bolt.setId(rs.getInt(1)); // else throw new Exception ( "AutoIndex" );
            }
        } catch (Exception e) {
            throw e;
        } finally {
     ...
     *
     */
    public void name(JDBC jdbc, Bolt bolt, int id, String name) {
        bolt.query(jdbc, BoltYard.fieldQuery(bolt, id, "name", name));
    }

    /**
     *
     */
    public void uuid(JDBC jdbc, Bolt bolt, int id, String uuid) {
        bolt.query(jdbc, BoltYard.fieldQuery(bolt, id, "uuid", uuid));
    }

    /**
     *
     */
    public boolean update(JDBC jdbc, Bolt bolt) {
        return update(jdbc, bolt, true);
    }
    ...
    public boolean update(JDBC jdbc, Bolt bolt, boolean ts) {
        ...
        try {
            ...
            ps = jdbc.prepareStatement();
            return update$(jdbc, bolt, ps, ts);
