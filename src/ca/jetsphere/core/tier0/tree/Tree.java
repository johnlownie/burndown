...
//    sb.append ( "<span class=\"node-draggable-pointer\">&zwnj;</span>" );
    ...
    public int getChildCount() {
        return tree_children != null ? tree_children.size() : 0;
    }
    ...
    public int getCount(int depth) {
        ...
        for (int cc = 0; cc < getChildCount(); cc++) {
            count += getChild(cc).getCount(depth);
        }
    ...
    public void pasteBelow(Tree parent) {
        ...
        setOrdinal(parent.getChildCount());
        setDepth(parent.getDepth() + 1);
        setLineage(parent.getLineage() + parent.getId() + "/");
    }
    ...
    public void reorder(JDBC jdbc, Tree drag, Tree drop, boolean after) throws Exception {
        if (!tree_children.contains(drag)) {
            return;
        }

        if (!tree_children.contains(drop)) {
            return;
        }
        ...
        if (cc > tree_children.size()) {
            tree_children.add(drag);
        } else {
            tree_children.add(cc, drag);
        }
     ...
     *
     */
    public void setDepth(int tree_depth) {
        this.tree_depth = tree_depth;
    }

    public void setInsertType(int tree_insert_type) {
        this.tree_insert_type = tree_insert_type;
    }

    public void setSelected(int tree_selected) {
        this.tree_selected = tree_selected;
    }
