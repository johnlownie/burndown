...
abstract public class TreeMap extends BoltMap {
    ...
    public void dragDrop(JDBC jdbc, Tree drag, Tree drop, boolean after) throws Exception {
        if (drag == null || !drag.isValid() || drop == null || !drop.isValid()) {
            return;
        }
        ...
        if (isSibling(drag, drop)) {
            parent.reorder(jdbc, drag, drop, after);
        } else {
            move(jdbc, drop, drag);
        }
    }
     ...
     */
    public Bolt get1st() {
        return getRoot() != null ? getRoot() : super.get1st();
    }
    ...
    public void insertTree(JDBC jdbc, Tree parent) throws Exception {
        ...
        for (int cc = 0; cc < parent.getChildCount(); cc++) {
            Tree child = (Tree) parent.getChild(cc);
            child.paste(parent);
            insertTree(jdbc, child);
        }
    ...
    public void move(JDBC jdbc, Tree parent, Tree child) throws Exception {
        ...
        Tree xparent = getParent(child);
        if (xparent != null) {
            xparent.removeChild(child);
        }
        child.setParentId(parent.getId());
        child.setOrdinal(parent.getChildCount());
    ...
    public void treeify() {
        ...
        while (it.hasNext()) {
            ...
            if (0 >= tree.getParentId()) {
                ...
                Tree parent = (Tree) get(tree.getParentId());
                if (parent != null) {
