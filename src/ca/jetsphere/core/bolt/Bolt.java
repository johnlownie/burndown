...
abstract public class Bolt extends MetaBolt implements Comparable {
     ...
     *
     */
    public Bolt(JDBC jdbc, String query) {
        this();
        query(jdbc, query);
    }
     ...
     *
     */
    public void copySave(JDBC jdbc) throws Exception {
        copySave(jdbc, true);
    }
    ...
    static public Bolt getBolt(Class clazz, int id) {
        ...
        try {
            ...
            Constructor c = clazz.getDeclaredConstructor(new Class[]{int.class});
            x = c.newInstance(new Object[]{new Integer(id)});
     ...
     *
     */
    static public boolean isValid(Bolt bolt) {
        return bolt != null ? bolt.isValid() : false;
    }
    ...
    public void query(JDBC jdbc, String query, List parameters) {
        ...
        try {
            ...
            while (it.hasNext()) {
                ...
                if ("date".equalsIgnoreCase((String) pair.getKey())) {
                    ps.setDate(cc, (Date) pair.getValue());
                }

                if ("int".equalsIgnoreCase((String) pair.getKey())) {
                    ps.setInt(cc, (Integer) pair.getValue());
                }

                if ("string".equalsIgnoreCase((String) pair.getKey())) {
                    ps.setString(cc, (String) pair.getValue());
                }
     ...
     *
     */
    public void save(JDBC jdbc, boolean ts) throws Exception {
        if (getId() >= 0) {
            update(jdbc);
        } else {
            insert(jdbc);
