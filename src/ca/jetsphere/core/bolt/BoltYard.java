...
public class BoltYard {
     ...
     *
     */
    static public String idQuery(Bolt bolt, int id) {
        return "select * from jet_base_" + bolt.getKey() + " where " + bolt.getKey() + "_id = " + id;
    }
    ...
    static public Object getObject(ResultSet aResultSet, int column) {
        try {
            ...
            switch (aResultSet.getMetaData().getColumnType(column)) {
                case Types.CHAR:
                    return aResultSet.getString(column);
                case Types.VARCHAR:
                    return aResultSet.getString(column);
                ...
                case Types.VARBINARY:
                    return aResultSet.getString(column);
                case Types.NUMERIC:
                    return aResultSet.getBigDecimal(column);
                case Types.DECIMAL:
                    return aResultSet.getBigDecimal(column);

                case Types.BIT:
                    return new Boolean(aResultSet.getBoolean(column));
                case Types.TINYINT:
                    return new Integer(aResultSet.getInt(column));
                case Types.SMALLINT:
                    return new Integer(aResultSet.getInt(column));
                case Types.INTEGER:
                    return new Integer(aResultSet.getInt(column));
                case Types.BIGINT:
                    return new Long(aResultSet.getLong(column));
                case Types.REAL:
                    return new Float(aResultSet.getFloat(column));
                case Types.FLOAT:
                    return new Double(aResultSet.getDouble(column));
                case Types.DOUBLE:
                    return new Double(aResultSet.getDouble(column));
                case Types.BINARY:
                    return new Byte(aResultSet.getByte(column));
                case Types.LONGVARBINARY:
                    return new Byte(aResultSet.getByte(column));

                case Types.DATE:
                    return aResultSet.getDate(column);
                case Types.TIME:
                    return aResultSet.getTime(column);
                case Types.TIMESTAMP:
                    return aResultSet.getTimestamp(column);
    ...
    static public String[] getCaptions(ResultSet rs) {
        try {
            return getCaptions(rs.getMetaData());
        } catch (Exception e) {
        }
        return null;
    }
    ...
    static public String[] getCaptions(ResultSetMetaData rsmd) {
        try {
            ...
            for (int cc = 0; cc < rsmd.getColumnCount(); cc++) {
                captions[cc] = rsmd.getColumnLabel(cc + 1);
            }
    ...
    static public String getContainerKey(Bolt bolt) {
        try {
            ...
            Class clazz = getContainer(bolt);
            String key = clazz.getName();

            int cc = key.lastIndexOf(".");
            if (cc < 0) {
                cc = 0;
            } else {
                cc += 1;
            }
            key = key.substring(cc);
            ...
            for (cc = 0; cc < sb.length(); cc++) {
                ...
                sb.setCharAt(cc, Character.toLowerCase(c));
            }
