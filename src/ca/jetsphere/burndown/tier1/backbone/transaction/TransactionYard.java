...
public class TransactionYard {
    ...
    static public TransactionSession filter(TransactionSession transactions, int type_id) {
        ...
        while (it.hasNext()) {
            ...
            if (type_id == 1 && transaction.isFixed()) {
                filtered.add(transaction);
            } else if (type_id == 2 && !transaction.isFixed()) {
                filtered.add(transaction);
            }
        }
...
//    sb.append ( " and transaction_fitid = " + DockYard.quote ( transaction.getFitId () ) );
    ...
    static public String getQuery(int period_id, int account_id, int category_id, int type_id, boolean toggle, String start_date, String end_date) {
        ...
        if (toggle) {
            sb.append(" inner join jet_burndown_transaction t2 on t2.transaction_id != t1.transaction_id and t2.transaction_date = t1.transaction_date and t2.transaction_amount = t1.transaction_amount and t2.transaction_name = t1.transaction_name");
        }

        sb.append(" where t1.transaction_period_id = " + period_id);

        if (toggle) {
            sb.append(" and t2.transaction_period_id = " + period_id);
        }

        if (account_id > 0) {
            sb.append(" and t1.transaction_account_id = " + account_id);
        }

        if (type_id > 0) {
            sb.append(" and t1.transaction_type = " + type_id);
        }

        if (!DockYard.isWhiteSpace(start_date)) {
            sb.append(" and t1.transaction_date >= " + DockYard.quote(start_date));
        }

        if (!DockYard.isWhiteSpace(end_date)) {
            sb.append(" and t1.transaction_date <= " + DockYard.quote(end_date));
        }
    ...
    static public void parseAttachment(JDBC jdbc, TransactionSession transactions, FormFile formFile) {
        ...
        switch (formFile.getContentType()) {
            ...
            default:
                parseAttachmentOfx(jdbc, transactions, formFile);
        }
    }
    ...
    static public void parseAttachmentCsv(JDBC jdbc, TransactionSession transactions, FormFile formFile) {
        ...
        try {
            ...
            String[] parts = formFile.getFileName().split("-");
    ...
    static public void setUncategorized(JDBC jdbc, int category_id, String name) {
        ...
        TransactionSession transactions = new TransactionSession();
        transactions.query(jdbc, query);
