...
public class Transaction extends ca.jetsphere.burndown.tier0.backbone.transaction.Transaction {
    ...
    public Transaction(JDBC jdbc) {
        super();
        query(jdbc, "select * from jet_burndown_transaction");
    }

    /**
     *
     */
    public Transaction(JDBC jdbc, int transaction_id) throws Exception {
        super(jdbc, transaction_id);
    }
    ...
    public boolean equals(Object o) {
        ...
        if (getMemo() != null ? !getMemo().equals(that.getMemo()) : that.getMemo() != null) {
            return false;
        }
    ...
    public void foreign(JDBC jdbc) throws Exception {
        ...
        setCategoryName(DockYard.getToken(s, 1, "|"));
        setFixed(DockYard.toBoolean(DockYard.getToken(s, 2, "|")));
    }
    ...
    public Object get(String s) {
        if ("transaction_amount".equals(s)) {
            return DockYard.toMoney(getAmount());
        }

        if ("transaction_account_id".equals(s)) {
            return AccountYard.getNumber(getAccountId());
        }

        if ("transaction_category_id".equals(s)) {
            return getCategoryName();
        }

        if ("transaction_fixed".equals(s)) {
            return isFixed();
        }
        ...
        if ("possible_duplicate".equals(s)) {
            return TransactionYard.exists(this);
        }
...
//    result = 31 * result + ( getAmount() != 0 ? getAmount().hashCode() : 0 );
    ...
    public String getDateAsString() {
        return CalendarYard.getDateTimeFormat(getDate(), "yyyy-MM-dd");
    }
    ...
    public void setDateAsString(String date) {
        setDate(new java.sql.Date(CalendarYard.getDate(date, "yyyy-MM-dd").getTime()));
    }
