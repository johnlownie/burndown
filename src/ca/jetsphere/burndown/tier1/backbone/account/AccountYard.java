...
public class AccountYard {
    ...
    static public Account getOrCreate(JDBC jdbc, String number, String type, String bank_id) {
        ...
        if (!DockYard.isWhiteSpace(type)) {
            sb.append(" and account_type = " + DockYard.quote(type));
        }

        if (!DockYard.isWhiteSpace(bank_id)) {
            sb.append(" and account_bank_id = " + DockYard.quote(bank_id));
        }
